...
export async function scanSQLQuery(input: SQLQueryInput, customerId: string = 'anonymous'): Promise<SanitizedResponse> {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      return sanitizeSQLResult(internalResult, requestId, effective_session_id, 'scan_sql_query');
    }
...
export const scanSQLQueryTool = {
  name: 'scan_sql_query',
  description: `Protective check on SQL queries — catches injection or destructive operations before execution, so a malformed or injected query doesn't damage data on your watch.
